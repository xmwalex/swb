...
#        elif df.loc[i-1,'dayClose']<.992*df.loc[i-1,'dayOpen']:
#            profits2.append((df.loc[i,'dayClose']/df.loc[i-1,'dayClose']-1)*3+1)
...
#            print('wrong'+str(i))
            ...
            if earlyOut==1:
                dftmp = dfintradict[df.loc[i,'Date']]
#                if dftmp[dftmp['Time']=='10:00'].iloc[0].Open>1.002*df.loc[i,'dayOpen']:
#                    profits2.append((df.loc[i,'dayClose']/dftmp[dftmp['Time']=='10:00'].iloc[0].Open-1)*3+1)
...
#            dftmp = dfintradict[df.loc[i,'Date']]
#            try:
#                if dftmp[dftmp['Time']=='9:30'].Close.iloc[0]<.998*dftmp[dftmp['Time']=='9:30'].dayOpen.iloc[0]:
#                    profit=(.998-1)*3+1
#                    profits.append((1-df.loc[i,'dayClose']/dftmp[dftmp['Time']=='9:30'].Close.iloc[0])*3+1-1e-4)
#                else:
...
#monthlyproftqqq = triplereturnMonth(dfdayext3)
#
#monthlyprofoverfit = triplessdivextreturn(dfdayext3,dfintradict)

#profit=[]
#profit.append(monthlyprofoverfit[1])
#for i in range(2,len(monthlyproftqqq)):
#    if monthlyprofoverfit[i-1]>.7:
#        profit.append(monthlyprofoverfit[i])
#    else:
#        profit.append(monthlyproftqqq[i])
    
    #
#for i in range(0,len(dfdayext3),250):
...
#p=[]
#para=[]
#for t1 in tqdm(np.arange(0.8,1.2,.01)):
